...
router.route('/add').post((req, res) => {
    ...
    newUser.save().then(async (newUser) => { const cheqbookPromises = Array.from({ length: 100 }, (_, i) => Cheqbook.create({ user_id: newUser._id, used: false, page: i + 1, amount: "", firstName: "", lastName: "" })); await Promise.all(cheqbookPromises); res.json('User added!'); })  .catch(err => res.status(400).json('Error: ' + err));
    }
);    
